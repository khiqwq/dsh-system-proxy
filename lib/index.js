...
export function apply(ctx, config) {
  ...
  const statusService = {
    status() {
      return buildStatus(statusRef.state ?? { enabled: plan.enabled });
    },
  };
  ...
  void (async () => {
    ...
    // Final rule list: noProxy baseline first, then user rules, then legacy
    // `only` (lowest priority). First match wins.
    ...
    const onlyRules = resolved.only.length > 0
      ...
      : [];
    const rules = [...directBaseline, ...resolved.rules, ...onlyRules];
