...
export function apply(ctx, config) {
  ...
  const resolveCredential = async (ref) => {
    const credentials =
      (typeof ctx.get === "function" ? ctx.get("credentials") : undefined) ?? ctx.credentials;
    if (!credentials || typeof credentials.resolve !== "function") {
      throw new NetworkRouteError(
        `credential service is unavailable for "${ref}"`,
  ...
  const reload = async () => {
    ...
    if (!plan.enabled) {
      if (previousRuntime !== null) {
        ...
        if (runtime === previousRuntime) runtime = null;
      }
      ctx.logger.info(
    ...
    let resolved;
    try {
      resolved = await resolveProxyPlan(plan, { resolveCredential });
    } catch (error) {
    ...
    let nextRuntime;
    try {
      // Build the complete replacement before disturbing the last-known-good
      // runtime. The current global wrapper remains active while asynchronous
      // proxy/credential resolution runs.
      if (previousRuntime !== null) {
        previousRuntime.restore();
        runtime = null;
      }
      nextRuntime = installTransports({
      ...
      if (previousRuntime !== null) previousRuntime.close?.();
      runtime = nextRuntime;
      statusRef.state = {
  ...
  // Minimal test/legacy Cordis contexts may not implement optional injection.
  // In a real DSH host, install the interactive Settings namespace; otherwise
  // preserve the entry-config behavior used before Settings existed.
  ...
  if (typeof ctx.on === "function") {
    try {
      ctx.on("credentials/updated", (ref) => {
        const plan = normalizeConfig(currentConfig());
        if (plan.passwordRef === ref) {
          scheduleReload();
          return;
        }
        for (const spec of plan.proxies.values()) {
          if (spec.passwordRef === ref) {
            scheduleReload();
            break;
          }
        }
      ...
      ctx.logger.debug(`dsh-system-proxy: could not watch credential updates: ${error.message}`);
    }
  }
  scheduleReload();
}
