...
function normalizeProxySpec(spec, name) {
  if (typeof spec === "string") {
    ...
    return { url: value, source: undefined, username: undefined, password: undefined };
  }
  if (spec && typeof spec === "object" && !Array.isArray(spec)) {
    ...
    if (!source && url === "" && username === undefined && password === undefined) {
      throw new NetworkRouteError(
        `proxies["${name}"] must have a url or a source (system|env)`,
        "INVALID_PROXY",
      );
    }
    return { url, source, username, password };
  }
  throw new NetworkRouteError(
    `proxies["${name}"] must be a URL string or { url?, source?, username?, password? }`,
    "INVALID_PROXY",
  );
}
 ...
 */
export async function resolveProxyPlan(plan) {
  ...
  if (!plan.proxies.has("default") && (plan.mode !== "manual" || plan.url !== "" || plan.legacyMode)) {
    ...
    if (url) resolved.set("default", makeResolvedProxy("default", url));
  }
