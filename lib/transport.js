 ...
 *    TRACE by default) are replayed on another route;
 *  - the body must be buffered and under a size cap (streams / FormData are
 *    never replayed);
 *  - only connect-phase failures (DNS, connect, TLS-handshake-timeout, no
 *    headers within the window) trigger the switch — once a response has
 *    started, the request is never replayed;
...
/** Error codes that mean "we never reached the server's response". */
...
export function makeResolver({ rules, defaults, registry, logger }) {
  ...
  function withProxy(route, proxyName) {
    ...
    if (!entry) {
      if (!warnedMissing.has(proxyName)) {
        warnedMissing.add(proxyName);
        logger.warn(
          `dsh-system-proxy: proxy "${proxyName}" is not configured — requests fall back to direct`,
        );
      }
      return { kind: "direct" };
    }
  ...
  resolve.warnedMissing = warnedMissing;
  return resolve;
}
...
export function readRouteFacts(init, options) {
...
function cleanRouteHeaders(headers, options) {
  if (!headers || !options.trustRouteHeaders) return null;
...
/** Remove the route headers from the fetch init before dispatch. */
function sanitizeInit(init, options) {
  if (!options.trustRouteHeaders) return init;
  const headers = cleanRouteHeaders(init?.headers, options);
...
/** Whether a thrown error means "no response was ever received" (safe to retry). */
export function isConnectPhaseError(error) {
  let current = error;
  for (let depth = 0; depth < 4 && current; depth += 1) {
    if (current && typeof current === "object" && CONNECT_PHASE_CODES.has(current.code)) return true;
    current = current?.cause;
  }
  return false;
}

function extractMethod(init) {
  return (typeof init?.method === "string" && init.method.trim() !== "" ? init.method : "GET").toUpperCase();
}
...
async function followRedirects(input, init, target, hopsLeft, env) {
  const facts = { ...readRouteFacts(init, env.options), port: target.port };
  const route = env.resolve(target.hostname, facts);
  const dispatchInit = sanitizeInit(init, env.options);
  let response;
  switch (route.kind) {
    case "block":
  ...
  } else if (!bodyIsReplayable(init?.body)) {
...
async function fetchWithFallback(input, init, route, hostname, env) {
  ...
  const method = extractMethod(init);
  const safe = defaults.methods.includes(method) && bodyIsReplayable(init?.body);
  ...
  try {
    ...
    if (!timedOut && !isConnectPhaseError(error)) throw error;
    logger?.info?.(
...
function makeRequestWrapper(original, secure, resolve, registry, health, defaults, options) {
  return function request(...args) {
    ...
    const facts = {
      ...readRouteFacts({ headers: opts?.headers }, options),
