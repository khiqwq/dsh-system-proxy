      ...
      return h("li", { className: "dsp-card", "data-open": open, "data-settings-namespace": NAMESPACE },
        ...
        open && snapshot.status !== "loading" && h("form", { className: "dsp-body", onSubmit: (event) => { event.preventDefault(); if (!disabled && dirty) save(); } },
          h("div", { className: "dsp-grid" },
            ...
            h("label", { className: "dsp-label", htmlFor: "dsp-mode" }, t("mode")), h("select", { id: "dsp-mode", className: "dsp-control", value: draft.mode, disabled, onChange: (event) => edit("mode", event.target.value) }, ["auto", "env", "system", "manual"].map((mode) => h("option", { value: mode, key: mode }, mode))),
